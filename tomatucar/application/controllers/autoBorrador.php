...
class AutoBorrador extends CI_Controller{
	...
	function index($id= "0"){
		if($this->session->userdata('nombre'))
		{
			...
			$data['Titulo'] = 'Borrador de Auto';
			//ADD SELECT BY ID COCHE
			$coches = "";
			$coches = $this->Coche->select_detalleautoBorrador($id);
			$data['coches'] = $coches;
			$modelos = $this->Modeloauto->select_modelosautos($coches[0]['Marca']);
			//------------------------
			$data['marcas'] = $marcas;
			...
			$this->load->view('autoBorrador', $data);
